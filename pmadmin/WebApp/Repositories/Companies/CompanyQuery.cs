   ...
   public class CompanyQuery:QueryObject<Company>
    {
        public CompanyQuery WithAnySearch(string search)
        {
            if (!string.IsNullOrEmpty(search))
                And( x =>  x.Id.ToString().Contains(search) || x.Name.Contains(search) || x.Logo.Contains(search) || x.Domain.Contains(search) || x.Address.Contains(search) || x.City.Contains(search) || x.Province.Contains(search) || x.RegisterDate.ToString().Contains(search) );
            return this;
        }
               ...
               foreach (var rule in filters)
               {
											...
											if (rule.field == "Logo"  && !string.IsNullOrEmpty(rule.value))
						{
							And(x => x.Logo.Contains(rule.value));
											...
											if (rule.field == "Domain"  && !string.IsNullOrEmpty(rule.value))
						{
							And(x => x.Domain.Contains(rule.value));
						}
